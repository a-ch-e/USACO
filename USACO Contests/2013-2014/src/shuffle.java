...
December Contest Problem 3: The Bessie Shuffle
 */
...
public class shuffle {
	...
	public static void main(String[] args) throws IOException{
		...
		int[] inverseP = new int[M + 1];
		for(int i = 1; i <= M; i++){
			inverseP[Integer.parseInt(read.readLine())] = i;
		}
		System.out.println(Arrays.toString(inverseP));
		
		PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("shuffle.out")));
		for(int i = 0; i < Q; i++){
			int loc = Integer.parseInt(read.readLine());
			int reps = N - loc + 1;
			int x = 0;
			if(loc < M){
				x = M - loc;
			}
			for(int j = 0; j <= N - M && j < reps; j++){
