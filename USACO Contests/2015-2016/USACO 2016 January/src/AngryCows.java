...
public class AngryCows {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i = 0; i < N; i++){
			if(i > 0){
				int temp = Math.max(bales[i] - bales[i - 1], toGoLeft[i - 1] + 1);
				if(toGoLeft[i] == 0 || temp < toGoLeft[i])
					toGoLeft[i] = temp;
			}
			int next = Arrays.binarySearch(bales, bales[i] + toGoLeft[i] + 1);
			if(next < 0)
				next = - next - 2;
			int temp = i;
			while(temp < next){
				temp++;
				if(toGoLeft[temp] == 0 || toGoLeft[temp] > toGoLeft[i] + 1)
					toGoLeft[temp] = toGoLeft[i] + 1;
//				System.out.println(Arrays.toString(toGoLeft));
			}
		}
...
//			System.out.println(i);
			if(i < N - 1){
				int temp = Math.max(bales[i + 1] - bales[i], toGoRight[i + 1] + 1);
				if(toGoRight[i] == 0 || temp < toGoRight[i])
					toGoRight[i] = temp;
			}
			int next = Arrays.binarySearch(bales, bales[i] - toGoRight[i] - 1);
			if(next < 0)
				next = - next - 1;
			int temp = i;
			while(temp > next){
				temp--;
				if(toGoRight[temp] == 0 || toGoRight[temp] > toGoRight[i] + 1)
					toGoRight[temp] = toGoRight[i] + 1;
			}
		}
		System.out.println(Arrays.toString(toGoLeft));
		System.out.println(Arrays.toString(toGoRight));

		double best = Math.min(toGoRight[0], toGoLeft[N - 1]);
		best = Math.min(best, bales[N - 1] - bales[0]);
		for(int i = N - 1; i >= 0; i--){
			double dist = (bales[i] - (double)bales[binarySearchLeast(toGoLeft, toGoRight[i])]) / 2;
			double temp;
			if(dist < 0)
				temp = toGoRight[i];
			else
				temp = Math.max(toGoRight[i] + 1, dist);
//			System.out.println(temp);
	...
	private static int binarySearchLeast(int[] arr, int key){
		
		int ans = Arrays.binarySearch(arr, key);
		if(ans < 0){
			return (-ans - 2);
		}
		while(ans > 0 && arr[ans - 1] == key)
			ans--;
		return ans;
